...
| Complete Installer | 2.10.1 | Maintenance release on base `2.10.0`: restores automatic SPDX 2.3 SBOM generation (the build script had stopped calling the generator during an earlier reorganization) and updates the generator itself to match the current models/workflows manifest schema. No component pin, runtime, or product logic change (see `docs/releases/complete-installer-v2.10.1.md`). 2.10.0 (`KI-Stack-Complete-Installer-v2.10.0.zip`) is the latest published [GitHub Release](https://github.com/robertbackhaus-a11y/KI-Stack/releases/tag/2.10.0); see `docs/releases/complete-installer-v2.10.0.md` and the `## 2.10.0` changelog entry for that release's full feature set (central update checker, ComfyUI/Open WebUI supported-version contracts, LM-Studio autostart guard). |
