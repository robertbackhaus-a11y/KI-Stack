...
| Complete Installer | 2.9.0 | Adds `-ReplayComponent <component-id>` to deliberately re-run an already-compliant component's existing install/validate/backup path (currently `openwebui-visual-pack`/`openwebui-agent-pack`) without deleting files or changing a pinned version; regression- and real-target-validated, closing 2.8.0's guided-cutover First-Install acceptance boundary (see `docs/releases/complete-installer-v2.9.0.md`). 2.8.0 (`KI-Stack-Complete-Installer-v2.8.0.zip`, SHA256 `e5c228e32a341554a83e17ad002e4126461c46b93c941e07bb010591d4e48cfa`) remains the latest published [GitHub Release](https://github.com/robertbackhaus-a11y/KI-Stack/releases/tag/2.8.0); 2.9.0 is not yet tagged or published. Builds on 2.5.0's physical Greenfield installation and 2.6.0's RAG remote-rollback completion, both still current. |
