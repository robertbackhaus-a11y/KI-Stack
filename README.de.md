...
| Complete Installer | 2.10.1 | Wartungs-Release auf Basis `2.10.0`: stellt die automatische SPDX-2.3-SBOM-Erzeugung wieder her (der Build-Schritt hatte während einer früheren Umstrukturierung aufgehört, den Generator aufzurufen) und passt den Generator selbst an das aktuelle Models/Workflows-Manifest-Schema an. Keine Komponenten-Pin-, Runtime- oder Produktlogik-Änderung (siehe `docs/releases/complete-installer-v2.10.1.md`). 2.10.0 (`KI-Stack-Complete-Installer-v2.10.0.zip`) ist das aktuell veröffentlichte [GitHub Release](https://github.com/robertbackhaus-a11y/KI-Stack/releases/tag/2.10.0); siehe `docs/releases/complete-installer-v2.10.0.md` und den Changelog-Abschnitt `## 2.10.0` für dessen vollständigen Funktionsumfang (zentraler Update-Checker, ComfyUI-/Open-WebUI-Supported-Version-Verträge, LM-Studio-Autostart-Schutz). |
