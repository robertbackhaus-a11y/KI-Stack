...
$tempLogPath = Join-Path ([IO.Path]::GetTempPath()) 'KI-Stack-PythonGit-Starter.log'
$starterLogPath = $null
$finalExitCode = 1
...
function Write-SelfTestFailureSummary {
    ...
    $candidatePaths = @(
        ...
        (Join-Path ([IO.Path]::GetTempPath()) 'KI-Stack-PythonGit-SelfTest-latest.json')
    )
...
try {
    ...
    if ($Action -eq 'SelfTest') {
        ...
        Write-Host ''
        Write-Host '== Neuesten Preflight ermitteln ==' -ForegroundColor Cyan
