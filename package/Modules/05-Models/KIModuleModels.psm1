...
function Invoke-KIResumableHttpDownload {
    ...
    for ($attempt=1; $attempt -le $retryCount; $attempt++) {
        ...
        try {
            ...
            $request.Headers.UserAgent.ParseAdd('KI-Stack-Models-Workflows/1.4.5')
...
function Install-KIModuleModels {
    ...
    if(Test-Path -LiteralPath $markerPath -PathType Leaf){try{$existingMarker=Get-Content -LiteralPath $markerPath -Raw|ConvertFrom-Json -Depth 100;$markerCompliant=[string]$existingMarker.release-eq'KI-Stack-Models-Workflows-Execute-v1.4.5'}catch{$markerCompliant=$false}}
    ...
    if($workflowCompliant-and$externalModelCompliant){return [pscustomobject][ordered]@{success=$true;skipped=$true;message='AlreadyCompliant';data=[pscustomobject][ordered]@{release='KI-Stack-Models-Workflows-Execute-v1.4.5';modelsDownloaded=$false;workflowsChanged=$false}}}
    ...
    $marker=[pscustomobject][ordered]@{managedBy='KI-STACK-MODELS-WORKFLOWS-MANAGED';release='KI-Stack-Models-Workflows-Execute-v1.4.5';installedAt=(Get-Date).ToString('o');transactionId=[string]$Context.Transaction.transactionId;models=@($modelResults);workflows=@($catalog.workflows | ForEach-Object  file)}
