...
function Invoke-KIResumableHttpDownload {
    ...
    for ($attempt=1; $attempt -le $retryCount; $attempt++) {
        ...
        try {
            ...
            $request.Headers.UserAgent.ParseAdd('KI-Stack-Models-Workflows/1.3.7')
...
function Install-KIModuleModels {
    ...
    $marker=[pscustomobject][ordered]@{managedBy='KI-STACK-MODELS-WORKFLOWS-MANAGED';release='KI-Stack-Models-Workflows-Execute-v1.3.7';installedAt=(Get-Date).ToString('o');transactionId=[string]$Context.Transaction.transactionId;models=@($modelResults);workflows=@($catalog.workflows | ForEach-Object  file)}
