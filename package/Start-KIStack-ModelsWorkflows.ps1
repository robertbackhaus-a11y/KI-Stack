...
$tempLogPath = Join-Path ([IO.Path]::GetTempPath()) 'KI-Stack-ModelsWorkflows-Starter.log'
$starterLogPath = $null
$finalExitCode = 1
...
function Write-SelfTestFailureSummary {
    ...
    $candidatePaths = @(
        ...
        (Join-Path ([IO.Path]::GetTempPath()) 'KI-Stack-ModelsWorkflows-SelfTest-latest.json')
    )
