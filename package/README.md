...
Der zentrale Workflowkatalog führt drei kanonische zukünftige Workflows, ohne sie auszuliefern oder produktive Originaldateien zu verändern: `KREA-Realism-Official-Template.json`, `PONY-SDXL-Control-QuickTest-v2.json` und `WAN2.2-5B-Official.json`. KREA bleibt durch `flux1-krea-dev_fp8_scaled.safetensors`, `clip_l.safetensors`, `t5xxl_fp16.safetensors` und `ae.safetensors` blockiert. Pony bleibt durch `ponyDiffusionV6XL_v6StartWithThisOne.safetensors` blockiert. WAN 2.2 bleibt durch `umt5_xxl_fp8_e4m3fn_scaled.safetensors`, `wan2.2_ti2v_5B_fp16.safetensors` und `wan2.2_vae.safetensors` blockiert. Eine Anpassung von `MarkdownNote` erfolgt erst nach Bereitstellung der jeweiligen Modelle. `PONY-Control-QuickTest-v2.json`, `WAN2.2-5B-I2V-QuickTest.json` und `WAN2.2-5B-T2V-QuickTest.json` sind als ältere Dubletten beziehungsweise ersetzt gekennzeichnet. Es erfolgen keine automatischen Downloads optionaler Modelle und es werden keine Platzhalter-Workflows ausgeliefert.
