...
echo === KI-Stack Production Target Acceptance v1.0.8 - Audit ===
...
if not "%PACKAGE_VERSION%"=="1.0.8" (
  echo FEHLER: Falsche Paketversion. Erwartet 1.0.8, gefunden %PACKAGE_VERSION%.
  pause
  exit /b 1
)
title KI-Stack Production Target Acceptance v1.0.8 - Audit
set "PWSH="
...
"%PWSH%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0Invoke-KIStack-Target-Acceptance.ps1" -Mode Audit
set "EC=%ERRORLEVEL%"
echo.
if "%EC%"=="0" (echo Zielsystemaudit erfolgreich.) else (echo Zielsystemaudit fehlgeschlagen. Exitcode: %EC%)
echo.
pause
popd
exit /b %EC%
