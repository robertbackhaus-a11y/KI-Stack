[CmdletBinding()]
param(
    [string]$OutputDirectory=(Join-Path (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))) '_import\integration-v1.5.10'),
...
try{
    ...
    New-Item -ItemType Directory $OutputDirectory -Force|Out-Null;$zip=Join-Path $OutputDirectory 'KI-Stack-Integration-Execute-v1.5.10.zip';if(Test-Path $zip){Remove-Item $zip -Force}
