[CmdletBinding()]
param(
    [string]$OutputDirectory=(Join-Path (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))) '_import\integration-v1.5.9'),
...
$payload=Join-Path $PayloadDirectory $contract.fileName;$manifest=Join-Path $PayloadDirectory 'SEARXNG-CONTENT-MANIFEST.json'
if(-not(Test-Path $payload)-or(Get-Item $payload).Length-ne$contract.sizeBytes-or(Get-FileHash $payload -Algorithm SHA256).Hash.ToLowerInvariant()-ne$contract.sha256){throw'SearXNG payload contract mismatch'}
if(-not(Test-Path $manifest)){throw'SearXNG content manifest missing'}
...
try{
    ...
    New-Item -ItemType Directory (Join-Path $stage 'Payload') -Force|Out-Null;Copy-Item $payload (Join-Path $stage 'Payload') -Force;Copy-Item $manifest (Join-Path $stage 'Payload/CONTENT-MANIFEST.json') -Force
    ...
    New-Item -ItemType Directory $OutputDirectory -Force|Out-Null;$zip=Join-Path $OutputDirectory 'KI-Stack-Integration-Execute-v1.5.9.zip';if(Test-Path $zip){Remove-Item $zip -Force}
