[CmdletBinding()]
param([string]$OutputDirectory = (Join-Path (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))) '_import\openwebui-agent-pack-v1.8.0'))
...
$zip = Join-Path $output 'KI-Stack-OpenWebUI-Agent-Pack-v1.8.0.zip'
$sidecar = "$zip.sha256"
