...
try {
    switch ($Action) {
        'Execute' {
            ...
            [ordered]@{ version='1.9.0'; action='Execute'; passed=$true; baseModelId=$result.baseModelId; backupPath=$result.backupPath; operations=$result.actions; chatModelFilter=$result.chatModelFilter } | ConvertTo-Json -Depth 10
        }
        'Validate' {
        ...
        'Rollback' {
            ...
            [ordered]@{ version='1.9.0'; action='Rollback'; passed=$true; backupPath=$BackupPath } | ConvertTo-Json
        }
    }
}
finally {
