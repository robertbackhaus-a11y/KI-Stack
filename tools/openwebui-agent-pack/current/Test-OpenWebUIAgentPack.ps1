...
if ($version -ne '1.8.2' -or $config.version -ne $version -or $manifest.version -ne $version) { $failures.Add('Versionskonsistenz') }
...
$report = [ordered]@{ version='1.8.2'; action='SelfTest'; passed=($failures.Count -eq 0); checks=8; failures=@($failures) }
