...
if ($version -ne '1.8.8' -or $config.version -ne $version -or $manifest.version -ne $version) { $failures.Add('Versionskonsistenz') }
...
foreach ($contract in @('ki_stack_generate_image','ki_stack_generate_video','2.0.5-rc3')) {
    if (-not $moduleText.Contains($contract)) { $failures.Add("Visual-Tool-Vertrag: $contract") }
}
$report = [ordered]@{ version='1.8.8'; action='SelfTest'; passed=($failures.Count -eq 0); checks=13; failures=@($failures) }
