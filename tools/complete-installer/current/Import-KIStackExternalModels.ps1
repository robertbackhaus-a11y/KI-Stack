...
$archive=Get-ChildItem -LiteralPath $payloadDirectory -File -Filter 'KI-Stack-Models-Workflows-Execute-v1.4.8.zip'|Select-Object -First 1
if(-not$archive){throw "Models-/Workflows-Payload 1.4.8 fehlt: $payloadDirectory"}
$extractRoot=Join-Path ([IO.Path]::GetTempPath()) ('KIStack-Models-1.4.8-'+[guid]::NewGuid().ToString('N'))
try{
