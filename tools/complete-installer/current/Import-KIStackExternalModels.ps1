...
$archive=Get-ChildItem -LiteralPath $payloadDirectory -File -Filter 'KI-Stack-Visual-Models-Workflows-v2.0.1.zip'|Select-Object -First 1
if(-not$archive){throw "Visual-Models-/Workflows-Payload 2.0.1 fehlt: $payloadDirectory"}
$extractRoot=Join-Path ([IO.Path]::GetTempPath()) ('KIStack-Visual-Models-2.0.1-'+[guid]::NewGuid().ToString('N'))
try{
    ...
    $arguments=@('-SourcePath',$SourcePath,'-TargetRoot',$TargetRoot,'-LmStudioTargetRoot',$LmStudioTargetRoot,'-StateRoot',$StateRoot)
