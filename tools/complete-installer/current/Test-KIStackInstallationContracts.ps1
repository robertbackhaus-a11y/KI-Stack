...
$required=@('README.md','README.de.md','Start-KIStack-Installer.cmd','Start-KIStackCompleteInstaller.ps1','Resume-KIStack-Installer.cmd','Status-KIStack.cmd','Lifecycle/Status-KIStack-Interactive.cmd')
...
$builder=Get-Content -LiteralPath (Join-Path $PackageRoot 'New-KIStackCompleteInstallerArchive.ps1') -Raw
...
$passed=$missing.Count-eq0-and$builder.Contains("'LICENSE','NOTICE','THIRD_PARTY_NOTICES.md','MODEL_LICENSES.md'")-and$startCmd.Contains('Start-KIStackCompleteInstaller.ps1')-and$resumeCmd.Contains('-Resume -TransactionId')-and$statusCmd.Contains('Lifecycle\Status-KIStack-Interactive.cmd')-and$startScript.Contains('-Verb RunAs')-and$startScript.Contains('if($needsOpenWebUI.Count)')-and$apiPrompts-eq1-and$core.Contains('OpenWebUIApiToken')-and$core.Contains('apiKeyStored=$false')-and(@($cmdContract|Where-Object{-not$_.crlf-or$_.bom}).Count-eq0)
[pscustomobject]@{passed=$passed;checks=[ordered]@{requiredFiles=($missing.Count-eq0);powershell7=($startCmd.Contains('PowerShell 7')-and$resumeCmd.Contains('PowerShell 7'));uacSingleElevation=($startScript.Contains('-Verb RunAs')-and$startScript.Contains('if($Elevated)'));resumeContract=($resumeCmd.Contains('-Resume -TransactionId')-and$core.Contains('Resume erfordert TransactionId.'));statusReadOnly=$statusCmd.Contains('Lifecycle\\Status-KIStack-Interactive.cmd');apiKeyFixture=[ordered]@{alreadyCompliantPrompts=0;agentAndImagePrompts=$apiPrompts;storedInReportsOrResume=$false};cmdFiles=$cmdContract};missing=$missing}|ConvertTo-Json -Depth 20
