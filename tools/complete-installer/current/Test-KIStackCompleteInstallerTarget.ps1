...
[pscustomobject]@{version='2.2.9';passed=([bool]$validate.health.passed-and[bool]$validate.operations.passed);auditReadOnly=(-not$audit.mutatesTarget);existingInstallation=$true;health=$validate.health;operations=$validate.operations;status='TargetValidated'}|ConvertTo-Json -Depth 30
