...
# reconciliation-skip bug), while Contracts/COMPONENTS.json's real pin is '1.6.15'. Confirmed
...
try {
    ...
    Write-KICompleteJson $stateStatePath ([ordered]@{ schemaVersion = '1.0'; status = 'ValidatedExistingInstallation'; completeInstallerVersion = '2.18.0'; validatedAtUtc = [DateTime]::UtcNow.ToString('o'); components = [ordered]@{ 'mcp-runtime' = $mcpVersion }; evidence = [ordered]@{ containsSecrets = $false } })
