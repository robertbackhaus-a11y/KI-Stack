...
$stage=Join-Path $OutputDirectory 'KI-Stack-Complete-Installer-v2.2.8'
...
$zip=Join-Path $OutputDirectory 'KI-Stack-Complete-Installer-v2.2.8.zip';if(Test-Path $zip){Remove-Item $zip -Force}
...
$sbom=Join-Path $OutputDirectory 'KI-Stack-Complete-Installer-v2.2.8.spdx.json'
& (Join-Path $repositoryRoot 'scripts\New-KIStackSpdxSbom.ps1') -PackageName 'KI-Stack Complete Installer' -PackageVersion '2.2.8' -ZipPath $zip -OutputPath $sbom -ModelsManifestPath (Join-Path $repositoryRoot 'package\Manifests\models.manifest.json') -ComponentsPath (Join-Path $PSScriptRoot 'Contracts\COMPONENTS.json')|Out-Null
