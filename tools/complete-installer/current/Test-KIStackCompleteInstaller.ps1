...
if ($manifest.version -ne '2.5.0' -or $manifest.baseVersion -ne '2.4.0') { $fail.Add('Version contract') }
...
if($ragComponent.Count-ne1-or$ragComponent.version-ne'0.3.0'-or-not[bool]$ragComponent.installable){$fail.Add('RAG component')}
...
try {
    ...
    $null=Update-KICompleteComponentState -Plan $statePlan -TargetRoot $planTarget -CompleteVersion '2.5.0'
    ...
    $null=Update-KICompleteComponentState -Plan $orphanStatePlan -TargetRoot $planTarget -CompleteVersion '2.5.0'
