...
if($modelsUpgradeSteps.Count-ne 1-or[string]$modelsUpgradeSteps[0].id-ne'models-workflows'-or[string]$modelsUpgradeSteps[0].plannedMode-ne'Upgrade'){$fail.Add('Models-1.4.1-to-1.4.2-Plan')}
$payloads=Get-Content (Join-Path $PackageRoot 'Contracts/PAYLOADS.json') -Raw|ConvertFrom-Json
$workflowModels=@($payloads.external|Where-Object{$_.PSObject.Properties.Name-contains'category'-and[string]$_.category-eq'models-workflows-1.4.2'})
$manualIds=@($payloads.externalManualDependencies)
if($workflowModels.Count-ne 8-or[long]($workflowModels|Measure-Object sizeBytes -Sum).Sum-ne47356936991-or[bool]$payloads.offline-or$manualIds.Count-ne7-or'flux-ae'-notin$manualIds){$fail.Add('Models-1.4.2-External-Payload-Contract')}
...
[pscustomobject]@{passed=($fail.Count-eq0);version='2.2.2';checks=34;fixtures=$plans.Keys;failures=$fail}|ConvertTo-Json -Depth 20
