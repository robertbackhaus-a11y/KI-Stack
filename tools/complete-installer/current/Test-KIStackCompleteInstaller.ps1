...
$workflowModels=@($payloads.external|Where-Object{$_.PSObject.Properties.Name-contains'category'-and[string]$_.category-eq'models-workflows-1.4.8'})
$manualIds=@($payloads.externalManualDependencies)
if($workflowModels.Count-ne 8-or[long]($workflowModels|Measure-Object sizeBytes -Sum).Sum-ne47356936991-or[bool]$payloads.offline-or$manualIds.Count-ne7-or'flux-ae'-notin$manualIds-or@($payloads.lmStudioModel.files).Count-ne2){$fail.Add('Models-1.4.8-External-Payload-Contract')}
...
[pscustomobject]@{passed=($fail.Count-eq0);version='2.2.8';checks=34;fixtures=$plans.Keys;failures=$fail}|ConvertTo-Json -Depth 20
