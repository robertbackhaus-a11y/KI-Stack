...
if(([string]($components.components|Where-Object id -eq 'models-workflows').version)-ne'1.4.0'){$fail.Add('Models-Workflows-Version')}
$fixtures=@{FreshInstall=@{};Upgrade=@{'foundation-runtime'='1.0.9';'comfyui'='1.2.1';'integration'='1.5.8'};ModelsUpgrade=@{'foundation-runtime'='1.0.9';'python-git'='1.1.5';'comfyui'='1.2.2';'models-workflows'='1.3.8';'applications'='1.4.10';'integration'='1.5.9';'cutover-runtime'='1.6.3';'production-recovery'='1.7.0-r7';'validation-gate'='1.0.2';'target-acceptance'='1.0.10';'openwebui-agent-pack'='1.8.3';'openwebui-image-pack'='1.9.1'};Repair=@{'foundation-runtime'='1.0.9';'comfyui'='damaged';'integration'='1.5.9'};Current=@{'foundation-runtime'='1.0.9';'python-git'='1.1.5';'comfyui'='1.2.2';'models-workflows'='1.4.0';'applications'='1.4.10';'integration'='1.5.9';'cutover-runtime'='1.6.3';'production-recovery'='1.7.0-r7';'validation-gate'='1.0.2';'target-acceptance'='1.0.10';'openwebui-agent-pack'='1.8.3';'openwebui-image-pack'='1.9.1'}}
$plans=[ordered]@{}
...
if($modelsUpgradeSteps.Count-ne 1-or[string]$modelsUpgradeSteps[0].id-ne'models-workflows'-or[string]$modelsUpgradeSteps[0].plannedMode-ne'Upgrade'){$fail.Add('Models-1.3.8-to-1.4.0-Plan')}
$payloads=Get-Content (Join-Path $PackageRoot 'Contracts/PAYLOADS.json') -Raw|ConvertFrom-Json
$workflowModels=@($payloads.external|Where-Object{$_.PSObject.Properties.Name-contains'category'-and[string]$_.category-eq'models-workflows-1.4.0'})
if($workflowModels.Count-ne 8-or[long]($workflowModels|Measure-Object sizeBytes -Sum).Sum-ne47356936991-or[bool]$payloads.offline){$fail.Add('Models-1.4.0-External-Payload-Contract')}
...
[pscustomobject]@{passed=($fail.Count-eq0);version='2.2.0';checks=34;fixtures=$plans.Keys;failures=$fail}|ConvertTo-Json -Depth 20
