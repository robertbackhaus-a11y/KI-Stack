...
if($v-ne'1.9.1'-or$c.version-ne$v-or$m.version-ne$v){$f.Add('Versionskonsistenz')};if($m.managedTool.id-ne'ki-stack-generate-image'-or$m.managedTool.openWebUIId-ne'ki_stack_generate_image'-or$m.managedTool.managedBy-ne'KI-STACK-OPENWEBUI-IMAGE-PACK'){$f.Add('Toolvertrag')}
...
@{version='1.9.1';action='SelfTest';passed=($f.Count-eq 0);checks=9;failures=@($f)}|ConvertTo-Json -Depth 10;if($f.Count){throw($f-join'; ')}
