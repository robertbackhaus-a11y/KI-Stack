[CmdletBinding()]param([string]$OutputDirectory=(Join-Path (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))) '_import\openwebui-image-pack-v1.10.0'))
...
$zip=Join-Path $output 'KI-Stack-OpenWebUI-Image-Pack-v1.10.0.zip';$sidecar="$zip.sha256";if(Test-Path $zip){Remove-Item $zip -Force}
...
$sbom=Join-Path $output 'KI-Stack-OpenWebUI-Image-Pack-v1.10.0.spdx.json'
& (Join-Path $repositoryRoot 'scripts\New-KIStackSpdxSbom.ps1') -PackageName 'KI-Stack OpenWebUI Image Pack' -PackageVersion '1.10.0' -ZipPath $zip -OutputPath $sbom -ModelsManifestPath (Join-Path $repositoryRoot 'package\Manifests\models.manifest.json')|Out-Null
