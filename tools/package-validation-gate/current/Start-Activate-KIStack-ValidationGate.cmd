...
echo === Validation Gate nativer Selbsttest ===
"%PWSH%" -NoProfile -ExecutionPolicy Bypass -File "%ROOT%Test-KIStack-Universal-ValidationGate.ps1"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" (
  echo Aktivierung abgebrochen. Selbsttest Exitcode: %RC%
  pause
  exit /b %RC%
)
echo === Validation Gate installieren ===
...
if "%RC%"=="0" echo Validation Gate ist verbindlich aktiviert.
pause
exit /b %RC%
