...
try {
    ...
    $embeddedPreflightTestPath = Join-Path $projectRoot 'Tests\Test-KIStackEmbeddedPreflight.ps1'
    if (-not (Test-Path -LiteralPath $embeddedPreflightTestPath -PathType Leaf)) {
        throw "Preflight-Erzeugungsregression fehlt: $embeddedPreflightTestPath"
    }
    $embeddedPreflightExitCode = Invoke-ChildPwsh -Arguments @(
        '-NoLogo',
        '-NoProfile',
        '-ExecutionPolicy', 'Bypass',
        '-File', $embeddedPreflightTestPath,
        '-ProjectRoot', $projectRoot
    )
    if ($embeddedPreflightExitCode -ne 0) {
        throw "Preflight-Erzeugungsregression ist fehlgeschlagen. Exitcode: $embeddedPreflightExitCode"
    }
    ...
    if ($Action -eq 'SelfTest') {
        ...
        elseif ([string]$config.starter.preflightSelectionMode -eq 'EmbeddedDefault') {
            $generatorPath = Join-Path $projectRoot 'New-KIStackEmbeddedPreflight.ps1'
            if (-not (Test-Path -LiteralPath $generatorPath -PathType Leaf)) {
                throw "Preflight-Generator fehlt: $generatorPath"
            }
            $generated = & $generatorPath -ProjectRoot $projectRoot
            $embeddedPreflightPath = [string]$generated.path
            if (-not (Test-Path -LiteralPath $embeddedPreflightPath -PathType Leaf)) {
                throw "Generierter Fortsetzungs-Preflight fehlt: $embeddedPreflightPath"
            }
            $preflight = Get-Item -LiteralPath $embeddedPreflightPath -ErrorAction Stop
            $preflightSource = 'Aus getrackten Paketquellen generierter Fortsetzungs-Preflight'
        }
        else {
            $preflight = Find-KILatestPreflight `
                -SearchRoots $searchRoots `
