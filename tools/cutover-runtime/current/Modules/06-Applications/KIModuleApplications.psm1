...
function Get-KIOpenWebUISecretKeyPath {
    # 2.18.1 hotfix (real, reproduced defect): Open WebUI's own bundled `serve` entrypoint
    # (open_webui/__init__.py, verified against the real installed open-webui==0.11.3 package)
    # falls back to `KEY_FILE = Path.cwd() / '.webui_secret_key'` whenever WEBUI_SECRET_KEY is not
    # already set as an environment variable. None of the KI-Stack starter chain (Complete
    # Installer's central Start-KIStack.cmd, the cutover core, this Open WebUI starter itself)
    # ever changed directory, so that Path.cwd() was whatever the outermost caller's current
    # directory happened to be -- e.g. C:\Windows\System32 for an elevated shell opened the usual
    # Windows way -- and Open WebUI then tried (and, without admin rights, failed) to write its
    # persistent secret key there. open-webui's separate WEBUI_SECRET_KEY_FILE env var is only
    # honored by its start.sh/start_windows.bat launchers, neither of which this starter uses, so
    # it is not a viable fix here; this starter instead sets WEBUI_SECRET_KEY itself, from a
    # persistent, TargetRoot-scoped file, which the code path actually in use does honor.
    param([Parameter(Mandatory)][string]$TargetRoot)
    Join-Path $TargetRoot 'state/openwebui/.webui_secret_key'
}

function Install-KIOpenWebUISecretKey {
    # Ensures the persistent secret key file from Get-KIOpenWebUISecretKeyPath exists, generating
    # a cryptographically random one only the first time -- never regenerated afterward, so
    # existing sessions/JWTs survive across every later Start/Repair/Upgrade. Backward
    # compatibility: the Desktop "KI-Stack starten.lnk" shortcut has always launched with
    # WorkingDirectory=$TargetRoot (see CompleteInstaller.psm1's Install-KICompleteOperations), so
    # a pre-fix installation's own key -- generated by Open WebUI's own Path.cwd() fallback --
    # would legitimately sit directly at <TargetRoot>\.webui_secret_key; that one specific,
    # verified legitimate path is reused if present, so existing sessions are not silently
    # invalidated. No other historical location is guessed at, and nothing under a Windows system
    # directory is ever treated as something to adopt.
    param([Parameter(Mandatory)][string]$TargetRoot)
    $keyPath = Get-KIOpenWebUISecretKeyPath -TargetRoot $TargetRoot
    if (Test-Path -LiteralPath $keyPath -PathType Leaf) {
        return [pscustomobject][ordered]@{ path = $keyPath; created = $false; migratedFrom = $null }
    }
    $keyDirectory = Split-Path -Parent $keyPath
    if (-not (Test-Path -LiteralPath $keyDirectory -PathType Container)) {
        New-Item -ItemType Directory -Path $keyDirectory -Force | Out-Null
    }
    $legacyPath = Join-Path $TargetRoot '.webui_secret_key'
    if (Test-Path -LiteralPath $legacyPath -PathType Leaf) {
        $existingKey = (Get-Content -LiteralPath $legacyPath -Raw)
        if (-not [string]::IsNullOrWhiteSpace($existingKey)) {
            [IO.File]::WriteAllText($keyPath,$existingKey.Trim(),[Text.UTF8Encoding]::new($false))
            return [pscustomobject][ordered]@{ path = $keyPath; created = $true; migratedFrom = $legacyPath }
        }
    }
    $randomBytes = [byte[]]::new(32)
    [Security.Cryptography.RandomNumberGenerator]::Fill($randomBytes)
    $secretKey = [Convert]::ToBase64String($randomBytes)
    [IO.File]::WriteAllText($keyPath,$secretKey,[Text.UTF8Encoding]::new($false))
    return [pscustomobject][ordered]@{ path = $keyPath; created = $true; migratedFrom = $null }
}

function Get-KIOpenWebUIStarterScriptContent {
    # Extracted from Install-KIModuleApplications, mirroring Get-KILMStudioStarterScriptContent,
    # so the generated batch contract (persistent secret key, stable working directory) is
    # directly testable without a full venv/package install.
    param(
        [Parameter(Mandatory)][string]$DataRoot,
        [Parameter(Mandatory)][AllowEmptyString()][string]$OpenAIBaseUrl,
        [Parameter(Mandatory)][AllowEmptyString()][string]$OpenAIKey,
        [Parameter(Mandatory)][string]$VenvPython,
        [Parameter(Mandatory)][string]$BindAddress,
        [Parameter(Mandatory)][string]$Port,
        [Parameter(Mandatory)][string]$TargetRoot,
        [Parameter(Mandatory)][string]$SecretKeyFilePath
    )
    $openWebUITemplate = @'
@echo off
setlocal EnableExtensions DisableDelayedExpansion
title KI-Stack Open WebUI
cd /d "__TARGET_ROOT__"
...
set /p WEBUI_SECRET_KEY=<"__SECRET_KEY_FILE__"
...
'@
    ...
    $openWebUIContent = $openWebUIContent.Replace('__TARGET_ROOT__',$TargetRoot)
    $openWebUIContent = $openWebUIContent.Replace('__DATA_ROOT__',$DataRoot)
    $openWebUIContent = $openWebUIContent.Replace('__OPENAI_BASE_URL__',$OpenAIBaseUrl)
    $openWebUIContent = $openWebUIContent.Replace('__OPENAI_KEY__',$OpenAIKey)
    $openWebUIContent = $openWebUIContent.Replace('__VENV_PYTHON__',$VenvPython)
    $openWebUIContent = $openWebUIContent.Replace('__BIND_ADDRESS__',$BindAddress)
    $openWebUIContent = $openWebUIContent.Replace('__PORT__',$Port)
    $openWebUIContent = $openWebUIContent.Replace('__SECRET_KEY_FILE__',$SecretKeyFilePath.Replace('/','\'))
    $openWebUIContent
}
...
function Install-KIModuleApplications {
    ...
    $targetRoot = Split-Path -Parent (Split-Path -Parent $moduleRoot)
    ...
    $secretKeyResult = Install-KIOpenWebUISecretKey -TargetRoot $targetRoot
    Write-KIApplicationsProgress -Context $Context -Step 'OpenWebUISecretKeyEnsured' -Data ([ordered]@{
        path=$secretKeyResult.path;created=[bool]$secretKeyResult.created;migrated=($null-ne$secretKeyResult.migratedFrom)
    })
    $openWebUIContent = Get-KIOpenWebUIStarterScriptContent -DataRoot $dataRoot -OpenAIBaseUrl ([string]$webConfig.openAIBaseUrl) -OpenAIKey ([string]$webConfig.openAIKey) -VenvPython $venvPython -BindAddress ([string]$webConfig.bindAddress) -Port ([string]$webConfig.port) -TargetRoot $targetRoot -SecretKeyFilePath $secretKeyResult.path
    ...
    $completeInstallerModulePath = Join-Path $targetRoot 'installer\complete\CompleteInstaller.psm1'
...
'@
    ...
    # 2.18.1 hotfix: this release string (also module.json and the pinned Contracts/COMPONENTS.json
    # 'applications' version) MUST be bumped every time this module's own generated-content contract
    # (e.g. the secret-key/working-directory fix above) changes -- Complete Installer's compliance
    # check for 'applications' compares its pin against exactly this string as read back from an
    # existing target's installation.json (Get-KICompleteInstalledVersion's default marker case), so
    # an unbumped release string here means an already-installed target is permanently reported
    # compliant/Skip and never actually receives the new generated content. Real, reproduced: the
    # 2.18.1 OpenWebUI secret-key fix shipped in this file without this bump and was confirmed, live,
    # to never reach an existing target for exactly this reason.
    $marker = [pscustomobject][ordered]@{
        managedBy = 'KI-STACK-APPLICATIONS-MANAGED'
        release = 'KI-Stack-Applications-Execute-v1.4.12'
...
function Validate-KIModuleApplications {
    ...
    $validateTargetRoot = Split-Path -Parent (Split-Path -Parent ([string]$applicationConfig.moduleRoot))
    foreach ($requiredPath in @(
        ...
        (Get-KIOpenWebUISecretKeyPath -TargetRoot $validateTargetRoot)
    )) {
