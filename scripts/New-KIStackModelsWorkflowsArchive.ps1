...
$stage=Join-Path $OutputDirectory 'KI-Stack-Models-Workflows-Execute-v1.4.8'
...
foreach($relative in @('en/KI-Stack-Technical-Documentation.md','de/KI-Stack-Technische-Dokumentation.md','en/KI-Stack-Operations-and-User-Guide.md','de/KI-Stack-Betriebs-und-Benutzerhandbuch.md','en/KI-Stack-Manual-Model-Provisioning.md','de/KI-Stack-Manuelle-Modellbereitstellung.md')){
...
Set-Content (Join-Path $stage 'VERSION') '1.4.8' -Encoding ASCII
@{schemaVersion='1.0';packageId='KI-STACK-MODELS-WORKFLOWS';name='KI-Stack Models / Workflows';version='1.4.8';status='TargetSystemValidated';workflows=@(@{file='KI-Stack-FLUX2-Text-to-Image-v1.3.8.json';sha256='b0c90e9fd38a4948fe97bbd7e95b2100261dc69b335794ba6c7db2fe4ff539db'},@{file='FLUX2-Klein-9B-OpenWebUI-API-FLAT.json';sha256='697ea261e1c62a8e32d775ee9cba5c5c5c3548c6bd082a63a84c71f53c3123a5'},@{file='KREA-Realism-Official-Template.json';sha256='344dc0a177b625d7bdde5292771a5455951178d6d498649cbb40f4e690216e65'},@{file='PONY-SDXL-Control-QuickTest-v2.json';sha256='7338036490ee1325062c75f10d89a46661cec6c43f17f5d1a035da5db2e68d40'},@{file='WAN2.2-5B-Official.json';sha256='7d4195f7a67d01829dd8a3d4c54f9b5fc857399a6f246c5b555b5a66848f27e6'});externalModelBytes=47356936991;externalLmStudioModelBytes=20676237728;containsModels=$false;offline=$false;containsSecrets=$false;containsPersonalPaths=$false;publicModelImporter=$true;sbomFormat='SPDX-2.3 JSON';documentation=@('docs/en/KI-Stack-Manual-Model-Provisioning.md','docs/de/KI-Stack-Manuelle-Modellbereitstellung.md')}|ConvertTo-Json -Depth 20|Set-Content (Join-Path $stage 'MANIFEST.json') -Encoding UTF8
@{schemaVersion='1.0';package='KI-Stack Models / Workflows';version='1.4.8';status='TargetSystemValidated';deterministicArchive=$true;offline=$false;containsModels=$false;externalModelBytes=47356936991;externalLmStudioModelBytes=20676237728;containsSecrets=$false;containsPersonalPaths=$false;publicModelImporter=$true;sbomFormat='SPDX-2.3 JSON';documentation='Bilingual'}|ConvertTo-Json -Compress|Set-Content (Join-Path $stage 'BUILD-REPORT.json') -Encoding UTF8
@{schemaVersion='1.0';package='KI-Stack Models / Workflows';version='1.4.8';status='TargetSystemValidated';validatedAtUtc='2026-07-23T12:00:00Z';results=@{previousFunctionalAcceptancePreserved=$true;centralModelImporterFixturePassed=$true;targetModelAuditPassed=$true;modelsDownloaded=$false;apiWorkflowUnchanged=$true;documentationParityPassed=$true;sbomGenerated=$true};containsRawTargetReport=$false;containsRenderedImages=$false;containsModels=$false;containsPersonalPaths=$false}|ConvertTo-Json -Depth 20 -Compress|Set-Content (Join-Path $stage 'VALIDATION-REPORT.json') -Encoding UTF8
...
$zip=Join-Path $OutputDirectory 'KI-Stack-Models-Workflows-Execute-v1.4.8.zip';if(Test-Path $zip){Remove-Item $zip -Force}
...
$sbom=Join-Path $OutputDirectory 'KI-Stack-Models-Workflows-Execute-v1.4.8.spdx.json'
& (Join-Path $PSScriptRoot 'New-KIStackSpdxSbom.ps1') -PackageName 'KI-Stack Models / Workflows' -PackageVersion '1.4.8' -ZipPath $zip -OutputPath $sbom -ModelsManifestPath (Join-Path $source 'Manifests\models.manifest.json')|Out-Null
[pscustomobject]@{zip=$zip;sizeBytes=(Get-Item $zip).Length;sha256=$hash;sbom=$sbom}
