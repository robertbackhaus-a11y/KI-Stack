...
$stage=Join-Path $OutputDirectory 'KI-Stack-Models-Workflows-Execute-v1.4.9'
...
$zip=Join-Path $OutputDirectory 'KI-Stack-Models-Workflows-Execute-v1.4.9.zip';if(Test-Path $zip){Remove-Item $zip -Force}
...
$sbom=Join-Path $OutputDirectory 'KI-Stack-Models-Workflows-Execute-v1.4.9.spdx.json'
& (Join-Path $PSScriptRoot 'New-KIStackSpdxSbom.ps1') -PackageName 'KI-Stack Models / Workflows' -PackageVersion '1.4.9' -ZipPath $zip -OutputPath $sbom -ModelsManifestPath (Join-Path $source 'Manifests\models.manifest.json')|Out-Null
