...
try {
    ...
    $completeVersionsOk=([string]($completeComponents.components|Where-Object id -eq 'comfyui').version -eq '1.2.4' -and [string]($completeComponents.components|Where-Object id -eq 'models-workflows').version -eq '2.0.3' -and [string]($completeComponents.components|Where-Object id -eq 'integration').version -eq '1.5.12'-and[string]($completeComponents.components|Where-Object id -eq 'cutover-runtime').version-eq'1.6.16'-and[string]($completeComponents.components|Where-Object id -eq 'openwebui-visual-pack').version-eq'2.0.5'-and[string]($completeComponents.components|Where-Object id -eq 'openwebui-ballistics-pack').version-eq'1.0.0'-and[string]($completeComponents.components|Where-Object id -eq 'openwebui-agent-pack').version-eq'1.9.0'-and[string]($completeComponents.components|Where-Object id -eq 'rag').version-eq'0.4.0')
    Add-Result 'Complete Installer component versions' $completeVersionsOk 'ComfyUI=1.2.4; Visual Models/Workflows=2.0.3; Integration=1.5.12; Cutover=1.6.16; Visual Pack=2.0.5; optional Ballistics=1.0.0; Agent Pack=1.9.0; RAG=0.4.0'
