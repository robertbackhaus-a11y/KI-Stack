...
try {
    ...
    $gitFreePackages=@(
        ...
        @{name='Complete Installer';root='tools/complete-installer/current';version='2.2.5'}
    )
    ...
    $completeVersionsOk=([string]($completeComponents.components|Where-Object id -eq 'comfyui').version -eq '1.2.2' -and [string]($completeComponents.components|Where-Object id -eq 'models-workflows').version -eq '1.4.5' -and [string]($completeComponents.components|Where-Object id -eq 'integration').version -eq '1.5.9'-and[string]($completeComponents.components|Where-Object id -eq 'openwebui-ballistics-pack').version-eq'1.0.0')
    Add-Result 'Complete Installer component versions' $completeVersionsOk 'ComfyUI=1.2.2; Models/Workflows=1.4.5; Integration=1.5.9; optional Ballistics=1.0.0'
