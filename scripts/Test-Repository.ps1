...
try {
    ...
    $completeVersionsOk=([string]($completeComponents.components|Where-Object id -eq 'comfyui').version -eq '1.2.4' -and [string]($completeComponents.components|Where-Object id -eq 'models-workflows').version -eq '2.0.1' -and [string]($completeComponents.components|Where-Object id -eq 'integration').version -eq '1.5.10'-and[string]($completeComponents.components|Where-Object id -eq 'cutover-runtime').version-eq'1.6.5'-and[string]($completeComponents.components|Where-Object id -eq 'openwebui-visual-pack').version-eq'2.0.5-rc2'-and[string]($completeComponents.components|Where-Object id -eq 'openwebui-ballistics-pack').version-eq'1.0.0')
    Add-Result 'Complete Installer component versions' $completeVersionsOk 'ComfyUI=1.2.4; Visual Models/Workflows=2.0.1; Integration=1.5.10; Cutover=1.6.5; Visual Pack=2.0.5-rc2; optional Ballistics=1.0.0'
