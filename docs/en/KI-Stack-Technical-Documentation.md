# KI-Stack 2.19.0 technical documentation

KI-Stack is a transactional Windows local-AI stack. Complete Installer `2.19.0` is the current published GitHub Release.
...
- **Desktop Control MCP wiring**: activated in 2.19.0 (see the release note above) as a flat `ui_*` tool surface on the existing MCP Runtime; a formal decision on whether to gate it behind a dedicated capability/profile, versus every existing `server:mcp:ki-stack-mcp-runtime`-bound profile gaining it structurally, remains open.
