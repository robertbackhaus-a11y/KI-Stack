...
| Agent / Tools | OpenWebUI Agent Pack; Codex Local; MCP Runtime `0.1.0` -- native MCP over `streamable-http` at `http://127.0.0.1:8021/mcp`, registered as `server:mcp:ki-stack-mcp-runtime` in OpenWebUI's own `tool_server.connections` (filesystem, process, and command execution via the vendored `open-terminal[mcp]` extra, pinned to `0.11.34`), authenticated with a single persistent, DPAPI-protected local API key -- this is the **default** path for every terminal-capable profile since 2.15 Phase 6. Open Terminal `0.1.0` (the same local `http://127.0.0.1:8000` OpenAPI backend as before) remains fully installed and runnable as an explicit, deactivatable fallback for 2.15, not removed. OpenWebUI remains the interface; neither Open Terminal nor MCP Runtime has a primary UI of its own |
| Integration | SearXNG, nginx, Valkey; WSL2/Debian where a component needs a Linux-side service |
| Lifecycle | Central `Start-KIStack.cmd`/`Stop-KIStack.cmd`/`Status-KIStack.cmd` cover every component, Open Terminal and MCP Runtime included; the Complete Installer manages Install/Upgrade/Repair/Skip per component; no Docker anywhere in the stack |
...
- **Automatic OpenWebUI tool-server registration**: both Open Terminal (`terminal_server.connections`) and MCP Runtime (`tool_server.connections`) still require a one-time manual registration under OpenWebUI's own Admin Settings -> Tools; the Complete Installer's Install/Upgrade/Repair actions provision and start each component but do not perform this registration step themselves. A credential-drift repair action exists for Open Terminal's registration (`Invoke-KIStackOpenTerminal.ps1 -Action SyncRegistrationCredential`, 2.15 Phase 7) and a full rotation path exists for the shared OpenWebUI admin bootstrap credential (`Initialize-KIStackOpenWebUICredential.ps1 -Rotate`), but the initial registration itself is still manual.
- **Legacy Open Terminal removal**: deferred beyond 2.15 -- MCP Runtime is the default for terminal-capable profiles, but Open Terminal is deliberately kept installed, supported, and reachable as a fallback; no hard cutover in 2.15.
